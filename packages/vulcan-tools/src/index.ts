...
import { registerVulcanHostBootstrapService } from "./host-bootstrap.js";
...
export default definePluginEntry({
  ...
  register(api) {
    ...
    // Register the shared host bootstrap service first so local installs can autostart vulcan-host before tool traffic arrives.
    // 先注册共享 host 启动服务，让本地安装能够在工具流量到来前自动拉起 vulcan-host。
    registerVulcanHostBootstrapService(api, config);
